...
namespace zupzip.demo.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Disassociate(string loginProvider, string providerKey) {
            ...
            if (result.Succeeded) {
                await UpdateExternalClaimData(User.Identity.GetUserId(), loginProvider);
        ...
        public async Task<ActionResult> LinkLoginCallback() {
            ...
            if (result.Succeeded) {
                await UpdateExternalClaimData(User.Identity.GetUserId());
        ...
        private async Task SignInAsync(ApplicationUser user, bool isPersistent) {
            ...
            await StoreExternalClaims(identity);
        ...
        #region External Claims

        private async Task UpdateExternalClaimData(string userId, string loginProvider = "") {
            var user = await UserManager.FindByIdAsync(userId);
            var userIdentity = await UserManager.CreateIdentityAsync(user, DefaultAuthenticationTypes.ApplicationCookie);

            userIdentity = string.IsNullOrWhiteSpace(loginProvider)
                                    ? await StoreExternalClaims(userIdentity)
                                    : await RemoveExternalClaims(userIdentity, loginProvider);
